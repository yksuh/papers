...
For each field referenced by the predicates, 
we then build a set of distinct ranges on that field. 
...
The whole range set then becomes a kind of two-dimensional array, called $R$, capturing 
the range representation of the gathered predicates. 
...
We then build another map, called $M$2, between $P$ and $R$. 
...
%1. Agrawal, R., Srikant, R.: Fast algorithms for mining association rules. In: Proceedings of the 20th International Conference on Very Large Databases. pp. 487–499. Morgan Kaufmann, San- tiago, Chile (1994)
%2. Bruce,K.B.,Cardelli,L.,Pierce,B.C.:Comparingobjectencodings.In:Abadi,M.,Ito,T.(eds.) Theoretical Aspects of Computer Software, Lecture Notes in Computer Science, vol. 1281, pp. 415–438. Springer-Verlag, Berlin Heidelberg New York (1997)
...
%5. Ribicˇre,M.,Charlton,P.:Ontologyoverview.MotorolaLabs,Paris(2002),[Online].Available:
...
%tiple granularities. ACM Transactions on Database Systems 22(2), 115–170 (1997)
...
%
%\bibitem{oneil:ssb}
%{P.~O’Neil, B.~O'Neil, and X.~Chen}, {The Star Schema Benchmark (SSB)},
