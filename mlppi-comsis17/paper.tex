...
contain the non-qualifying rows for $q$1, unfortunately, 
...
But via additional merges we may further make fewer partitions, 
...
But we empirically observed that the bound of $\BigO$($N${$\cdot$}$C${$\cdot$}$V$) 
...
the number of calls made in our experiments was much fewer than that bound, as 
it was almost rare for a range pair to involve all queries in a workload 
(as seen in Table~\ref{tab:workload_stat}).

In the following, we evaluate the proposed algorithm. 
