...
CREATE TABLE EH_Chosen_Runs_Ver1 AS -- Primary key is runid.
	...
	WHERE runid IN (610,615,616,617,618, 611,614) -- 5 db2 runs + 2 mysql runs
	ORDER BY runid;
...
CREATE VIEW EH_Analysis_S1_NQPV_Ver1 AS -- Primary key is a labshelf, so just 1 row!
...
CREATE VIEW EH_Analysis_S1_OQPV_Ver1 AS -- Primary key is a labshelf, so just 1 row!
...
CREATE VIEW EH_Analysis_S1_OUPV_Ver1 AS -- Primary key is a labshelf, so just 1 row!
...
DROP VIEW EH_Analysis_S2_II_Ver1;
CREATE VIEW EH_Analysis_S2_II_Ver1 AS 	-- Primary key is a query execution: (qeid)
	SELECT  *
	FROM EH_Analysis_S2_I_Ver1
	WHERE -- drop query executions below
	     (numphantomspresent = 0 AND stoppedprocesses=0);	
ALTER VIEW EH_Analysis_S2_II_Ver1 ADD PRIMARY KEY (qeid) DISABLE; 
