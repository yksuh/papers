...
We propose a novel structural causal model to explicate the
...
observe that \todo{Rick: check these}suboptimality prevalence
...
limitation of cost-based optimization.\todo{Rick:be more specific! Couple
  with contributions and conclusions}
...
\todo{Rick: For Science article, add history: IMS->System R,
  procedural->declarative, Selinger's query opt algorithm enabling rise of
  relational model}
\todo{Rick: For TODS, shorten history, probably remove SQL cites, just one
  sentence about central role of Selinger}
...
individual hash-join operator. \todo{Young: I don't understand what this
  sentence adds'' Rick: Yes, please delete the sentence.}Note that when two identical query plans
...
Section~\ref{sec:suboptimality}.\todo{Young: I don't see any such discussion
  in that section..., Rick: Please take a look at the definition of the strict monotonicity assumption. We discuss this there.}
This experiment involved 160 query instances, 32,000 \QatC s, and thus
320,000 QEs, requiring 1,672 cumulative hours.  The (very small\todo{Young:
  how much?, Rick: 1.55\%}) percentage % (16312+8215)/(995321+587546) = 1.55%
...
experiment was very fast.}\todo{Sabah:were any additional discontinous
  operators found? Rick: I don't know about this. I redirect this question to Sabah.}

\todo{Rick: Shouldn't this experiment  be before Experiment 3 (Exhaustive),
  because it used TTPv1?}The fifth experiment was for initial exploratory analysis of a prior
...
\todo{Young: these percentages  are different from those in Section 6.1. Why? Rick: A good catch. Those numbers are out-of-date: ones from the TODS metrology paper. I replaced them with the current numbers mentioned in this para. Now consistent.}In this Confirmatory Experiment, we observed 3,347 strict monotonicity
violations (0.74\%), and 1,966 relaxed monotonicity violations (0.43\%) (out
of a total of 452,684 pairs
of two Q@Cs having identical plans and query instance), \todo{Rick:merge
  somehow with Section 6.1:}which provides
...
\hline
Operators in \hbox{DBMS} & ---&  {\bf H1a:} 0.35 & {\bf H1b:} 0.27& {\bf H1c:} -0.09\\
Correlation names & --- & {\bf H2a:} 0.17 & {\bf H2b:} 0.46 &{\bf H2c:} 0.34\\
Presence of aggregate & -- & {\bf H2d:} 0.04 & {\bf H2e:} 0.10 & {\bf H2f:} 0.29\\
Presence of primary key & -- & {\bf H2g:} NS & {\bf H2h:} 0.15 & {\bf H2i:} 0.14\\
Presence of subquery & -- & {\bf H2j:} NS & {\bf H2k:} 0.26 & {\bf H2l:} 0.20\\
Skew & {\bf H3:} -0.05 & --- & ---  & ---\\
Repeats& {\bf H6a:} 0.62 & --- & --  & ---\\
CEPS &  {\bf H6b:} 0.61 & {\bf H4b:} 0.45 &  --- &{\bf H4a:} 0.72 \\
...
%&{\em }&{\em }&{\em }&{\em }\\
\hline
Correlation names & 0.17 & 0.15 &0.49 &0.39  & 0.36 & 0.27 \\
Presence of aggregate  & 0.04 & NS & 0.12 & NS & 0.31 & 0.22 \\
Presence of primary key  & -0.03 & NA & 0.06 & NA & 0.09 & NA \\
Presence of subquery  & 0.03 & NS & 0.18 & 0.17  & 0.14 & 0.13 
\end{tabular}
}
%\end{center}
}
...
of discontinuity\todo{Sabah: don't understand this ending phrase:}, with all of the independent variables. When
...
We did regressions on the causal variables for CEPS, number of repeats and for number of
...
We also ran a regression over the independent variables of the model that
...
explained by the independent and intevening variables in our model, the ten
variables within our five constructs of Optimizer Complexity, Schema
...
\todo{Sabah: update with final numbers:}
...
many decades of research and development. \todo{Rick: Please confirm this paragraph is still valid}
...
is naturally connected to the number of operators available\todo{Rick:
  connect with Section~\ref{sec:root} after Sabah updates that section; also
  discuss the exponential increase in CEPS possible}. A possible implication
...
\item This research indicates that aggregates are {\em not} a problem, so
  that aspect of query optimization is in good shape.\todo{Rick: check with
    Sabah's results and modify around here if needed.}
...
component.\todo{Rick:make consistent with list at the beginning of the paper}
\begin{itemize}
...
interactions.\todo{Rick:Check again}
...
of suboptimality\todo{Rick: state. Do we want to keep the following?}.  It is doubtful that any other factor, as yet unknown,
...
\item Applies a novel experiment over pairs of adjacent Q@Cs to show that
  such an upper bound is probably and may have already been reached with one
...
There are several directions this work could be take. \todo{RICK IS HERE}With the model refinements
we propose here, additional directed pointers to engineering efforts should
emerge.\todo{Rick:Don't say ``we'', but just say the research needs to be done}
...
\item[QSo] A query set consisting of 100 queries without aggregates (on Data
  Set C)\todo{Young: are these new queries, or drawn from prior query sets? Rick: New queries.}
